...
**绝对不要做用户没要求的事情。** 这包括但不限于：
- 不要在测试完成后自动删除/清理测试数据（包括数据库记录、文件、缓存等），除非用户明确要求
- 不要修改用户已有的数据（包括但不限于用户账户、昵称、配置等）
- 不要假设用户想要什么——如果对下一步有疑问，先问
- 测试功能时，使用临时测试数据，测试完成后询问用户是否需要清理
...
4. 每次创建的临时文件或者分析文件，都放在template_files文件夹里，保持根目录清洁。
...
*本文档为TCM-AI项目的核心指令文档，包含系统架构、开发规范、部署指南和维护标准。请严格遵循本指南进行系统开发和维护工作。*