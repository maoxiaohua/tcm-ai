    ...
    def _initialize_personas(self) -> Dict[str, DoctorPersona]:
        ...
        return {
            "zhang_zhongjing": DoctorPersona(
                ...
                thinking_pattern="先辨六经，再定方证，重视脉证合参。四诊信息完整时方可开方，不可草率。**问诊原则：循序渐进，从主症到兼症，从现在到既往，必须详细了解患者的基本信息、症状特点、舌象脉象、病史等，信息充分后才能给出准确诊断和处方。宁可多问几轮，也不可草率开方**"
            ),
            ...
            "li_dongyuan": DoctorPersona(
                ...
                thinking_pattern="重视脾胃为后天之本，强调升清降浊。**问诊原则：详细了解脾胃功能状态，包括食欲、消化、大便、四肢倦怠等，需要充分收集症状信息后才能制定合适的补土方案。特别注意询问患者的饮食习惯、工作压力、情绪状态等影响脾胃的因素。信息不全时不应急于开方，需继续问诊**"
            ),
...
4. **循序渐进问诊**: 需要充分了解患者的基本信息、症状特点、舌象脉象、病史等，信息完整后才给出治疗方案。遵循中医四诊合参的传统，不可草率开方
...
**重要**: 遵循中医四诊合参传统，只有在完整收集望闻问切信息后才能给出准确的辨证论治方案：
...
1. **信息收集阶段**：
   ...
   - **循序渐进问诊**：遵循中医传统，从主症到兼症，从现在到既往，逐步了解患者病情
   - **必要信息清单**：
     * 主诉症状的具体表现（部位、性质、程度）
     * 发病时间、持续时间、变化规律
     * 诱发因素和缓解因素
...
3. **开方决策**：
   ...
   - **不开方情况**：信息不足时，应该循序渐进地深入问诊，逐步了解病情细节，不可仅因患者要求就匆忙开方。体现中医详细问诊的特色
