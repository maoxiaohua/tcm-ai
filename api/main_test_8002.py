...
# 导入智能缓存系统
from intelligent_cache_system import IntelligentCacheSystem, get_cache_system, init_cache_system
from integrated_prescription_parser import parse_prescription_text
from prescription_checker import PrescriptionChecker
from prescription_ocr_system import get_ocr_system, PrescriptionOCRSystem
from tcm_knowledge_graph import TCMKnowledgeGraph
from famous_doctor_learning_system import FamousDoctorLearningSystem
import pickle
...
try:
    ...
    from tcm_doctor_personas import PersonalizedTreatmentGenerator
    from personalized_learning import PersonalizedLearningSystem, UserFeedback
    from doctor_mind_integration import EnhancedPersonalizedTreatmentGenerator, DoctorMindAPI
    from postgresql_knowledge_interface import get_hybrid_knowledge_system
    from medical_diagnosis_controller import medical_diagnosis_controller
...
# 初始化用户历史系统
try:
    from user_history_system import UserHistorySystem, user_history
...
# FAISS知识库路径配置
KNOWLEDGE_DB_PATH = os.getenv("KNOWLEDGE_DB_PATH", "/opt/tcm/knowledge_db")
...
if ENHANCED_SYSTEM_AVAILABLE:
    try:
        ...
        learning_system = PersonalizedLearningSystem(db_path="/opt/tcm/data/learning_db.sqlite")
...
try:
    ...
    async def mobile_modal_test():
        """移动端图片选择弹窗测试页面"""
        with open("/opt/tcm/mobile_modal_test.html", "r", encoding="utf-8") as f:
            content = f.read()
    ...
    async def wechat_verification_dynamic(filename: str):
        ...
        try:
            file_path = f"/opt/tcm/static/MP_verify_{filename}.txt"
    ...
    async def wechat_verification():
        """微信公众平台域名验证 - 向后兼容"""
        try:
            with open("/opt/tcm/static/MP_verify_wechat_verification.txt", "r") as f:
                content = f.read()
            return PlainTextResponse(content)
        except:
    ...
    async def robots_txt():
        """搜索引擎robots.txt"""
        try:
            with open("/opt/tcm/static/robots.txt", "r") as f:
                content = f.read()
            return PlainTextResponse(content)
        except:
...
async def doctor_portal():
    ...
    return FileResponse('static/doctor_portal.html')

@app.get("/doctor/thinking")  
async def doctor_thinking():
    """医生思维录入 - 友好URL"""
    from fastapi.responses import FileResponse
    return FileResponse('static/doctor_thinking_input.html')

@app.get("/doctor/thinking-v2")
async def doctor_thinking_v2():
    """医生思维录入V2 - 友好URL (推荐)"""
    from fastapi.responses import FileResponse
    return FileResponse('static/doctor_thinking_input_v2.html')

@app.get("/doctor/management")
async def doctor_management():
    """医生管理 - 友好URL"""
    from fastapi.responses import FileResponse
    return FileResponse('static/doctor_management.html')

@app.get("/doctor-test")
async def doctor_selection_test():
    """医生选择测试页面"""
    from fastapi.responses import FileResponse
    return FileResponse('static/doctor_test.html')
...
async def user_history_page():
    """用户历史记录页面"""
    from fastapi.responses import FileResponse
    return FileResponse('static/user_history.html')

@app.get("/qr_gallery")
async def qr_gallery_page():
    """二维码管理页面"""
    from fastapi.responses import FileResponse
    return FileResponse('static/qr_code_gallery.html')
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "version": "2.2.0",
        "deployment": "minimal_online_api_only",
        "services": {
            "qwen_api": bool(DASHSCOPE_API_KEY),
            "qwen_vl_api": bool(DASHSCOPE_API_KEY),
            "enhanced_system": ENHANCED_SYSTEM_AVAILABLE,
            "doctor_mind_system": DOCTOR_MIND_SYSTEM_AVAILABLE,
            "postgresql_hybrid_system": POSTGRESQL_HYBRID_AVAILABLE,
            "local_whisper": False,
...
async def test_upload_page():
    """提供图片上传测试页面"""
    with open("/opt/tcm/test_upload_simple.html", "r", encoding="utf-8") as f:
        html_content = f.read()
...
async def check_prescription_endpoint(
    ...
    try:
        ...
        # 使用基础处方解析器
        from prescription_checker import PrescriptionParser
...
async def prescription_checker_page():
    ...
    # 使用完整功能版本
    try:
        with open('/opt/tcm/prescription_checker_v2.html', 'r', encoding='utf-8') as f:
            html_content = f.read()
        ...
        try:
            ...
            # 最后尝试使用旧版本
            try:
                with open('/opt/tcm/prescription_checker_fixed.html', 'r', encoding='utf-8') as f:
                    html_content = f.read()
                return HTMLResponse(html_content)
            except FileNotFoundError:
                return HTMLResponse("""
                ...
                <body>
                    ...
                    <p><a href="/">返回主页</a> | <a href="/debug_status">系统状态</a></p>
                </body>
                </html>
                """)
...
async def unified_system_monitor():
    ...
    try:
        from fastapi.responses import FileResponse
        return FileResponse("/opt/tcm/static/unified_system_monitor.html")
    except Exception as e:
...
async def debug_layout():
    ...
    try:
        from fastapi.responses import FileResponse
        return FileResponse("/opt/tcm/static/debug_layout.html")
    except Exception as e:
