    ...
    try:
        ...
        if os.path.exists(learning_db_path):
            try:
                ...
                cursor = conn.execute('''
                    SELECT diagnosis, syndrome, prescription, confidence, learned_at 
                    FROM learned_cases 
                ...
                for row in cursor.fetchall():
                    details.append({
                        ...
                        'syndrome': row[1], 
                        'prescription': row[2],
                        'confidence': row[3] if row[3] else 0.0,
                        'learned_at': row[4]
                    })
                
                conn.close()
            except Exception as db_e:
                logger.warning(f"从学习数据库获取详情失败: {db_e}")
        
        # 如果没有数据库数据，检查是否有其他来源
        if not details:
            # 尝试从名医数据库获取
            famous_db_path = '/opt/tcm-ai/data/famous_doctors.db'
            if os.path.exists(famous_db_path):
                try:
                    conn = sqlite3.connect(famous_db_path)
                    cursor = conn.execute('''
                        SELECT tcm_diagnosis, syndrome_differentiation, prescription_content 
                        FROM medical_cases 
                        ORDER BY id DESC 
                        LIMIT 10
                    ''')
                    ...
                    for row in cursor.fetchall():
                        details.append({
                            'diagnosis': row[0] or '未知诊断',
                            'syndrome': row[1] or '未识别证候',
                            'prescription': row[2] or '处方信息不完整',
                    ...
                    logger.warning(f"从名医数据库获取详情失败: {db_e}")
